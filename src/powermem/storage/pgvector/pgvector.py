...
class PGVectorStore(VectorStoreBase):
    ...
    def insert(self, vectors: list[list[float]], payloads=None, ids=None) -> list[int]:
        ...
        data = [(vector, payload) for vector, payload in zip(vectors, json_payloads)]
        
        if PSYCOPG_VERSION == 3:
            with self._get_cursor(commit=True) as cur:
                # Use RETURNING to get auto-generated IDs
                cur.executemany(
                    f"INSERT INTO {self.collection_name} (vector, payload) VALUES (%s, %s) RETURNING id",
                    data,
                )
            ...
            with self._get_cursor(commit=True) as cur:
                # psycopg2: use execute_values with RETURNING
                execute_values(
                    cur,
                    f"INSERT INTO {self.collection_name} (vector, payload) VALUES %s RETURNING id",
                    data,
                )
                # Fetch all returned IDs
                generated_ids = [row[0] for row in cur.fetchall()]
