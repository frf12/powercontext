...
class OceanBaseVectorStore(VectorStoreBase):
    ...
    def _create_table_with_index_by_embedding_model_dims(self) -> None:
        ...
        cols = [
            ...
            Column(self.primary_field, BigInteger, primary_key=True, autoincrement=True),
            # Vector field
    ...
    def insert(self,
        ...
        Args:
            ...
            ids: Deprecated parameter (ignored), IDs are now auto-generated by database
            
        Returns:
            List[int]: List of auto-generated integer IDs
        """
        try:
            if not vectors:
                return []
            ...
            # Prepare data for insertion (without primary key field)
            data: List[Dict[str, Any]] = []
            for vector, payload in zip(vectors, payloads):
                record = self._build_record_for_insert(vector, payload)
                data.append(record)

            # Use transaction to ensure atomicity of insert and ID retrieval
            # Direct SQLAlchemy approach to get lastrowid from batch insert
            ...
            with self.obvector.engine.connect() as conn:
                with conn.begin():
                    ...
                    # Get first auto-generated ID from result
                    # In MySQL/OceanBase batch insert, IDs are consecutive
                    # lastrowid returns the first ID of the batch
                    first_id = result.lastrowid
                    
                    # Generate list of consecutive IDs for the batch
                    generated_ids = list(range(first_id, first_id + len(vectors)))
            
            logger.debug(f"Successfully inserted {len(vectors)} vectors, generated IDs: {generated_ids}")
    ...
    def _build_record_for_insert(self, vector: List[float], payload: Dict) -> Dict[str, Any]:
        ...
        record = {
            # Primary key is auto-generated, do not include it
            self.vector_field: (
